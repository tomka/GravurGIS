...
namespace MapTools
{
	...
	public class ShapeLib
	{
        ...
        {
            public IntPtr worldX;
            public IntPtr worldY;
            public IntPtr dispX;
            public IntPtr dispY;
            public IntPtr partListLengths;  // an array with length partCount with the length of the sub-lists, respectively
            public double scale;
            public int isPoint;
            public ShapeType shpType;
            public int partCount;   // the length of lists x,y and partListLengths
            public int nVertices;   // total count of vertices in this Shapefile
            public int shapeCount;
            public double minBX;
            public double minBY;
            public double maxBX;
            public double maxBY;
            public int isWellDefines;
        }
        ...
        /// <summary>
        /// This funtion let us generate fake unicodestrings out of ascii-strings
        /// </summary>
        /// <param name="input">The String you want to convert</param>
        /// <returns></returns>
        public static string convertToFakeAscii(string input)
        {
            int count = 0;
            ...
            if (length % 2 == 0)
                newByteArray = new byte[length + 2]; // we need a new double byte for the \0 at the end
            else
                newByteArray = new byte[length + 1]; // we only need one byte to fill up the current double byte

            for (int i = 0; i < byteArray.Length; i++) // kick out the 0x00's at every second position :)
                if (i % 2 == 0)
                {
            ...
            return Encoding.Unicode.GetString(newByteArray, 0, newByteArray.Length);
        }

        [DllImport("Shapelib.dll", CharSet = CharSet.Auto)]
        public static extern void deletePointObject([In] IntPtr sPointObject);

        /// <summary>
        /// The function generates as struct in the form of PointObject
        /// </summary>
        /// <param name="szShapeFile"></param>
        /// <param name="szAccess"></param>
        /// <param name="mapPanelWidth"></param>
        /// <param name="mapPanelHeight"></param>
        /// <param name="margin">The margin</param>
        /// <param name="recalculateScale">1 = true; 0 = false</param>
        /// <param name="oldScale">The old scale to be used</param>
        /// <param name="useOldScale">1 = true; 0 = false</param>
        /// <returns>A pointer to a PointObject struct</returns>
        [DllImport("Shapelib.dll", CharSet = CharSet.Auto)]
        public static extern IntPtr SHPGetPolyLineList([In] string szShapeFile,
            [In] string szAccess, [In] int mapPanelWidth, [In] int mapPanelHeight,
            [In] int margin, [In] int recalculateScale, [In] double oldScale,
            [In] int useOldScale, [In] int pointSize, [In] int getRawData);
		/// <summary>
        ...
        [DllImport("Shapelib.dll", CharSet = CharSet.Auto)]
        public static extern IntPtr SHPOpen([In] string szShapeFile, [In] string szAccess);

        [DllImport("Shapelib.dll", CharSet = CharSet.Auto)]
        public static extern IntPtr SHPOpenW([In] string szShapeFile, [In] string szAccess);

        public static IntPtr SHPOpenW_Wrapper(string szShapeFile, string szAccess) {
            return SHPOpen(convertToFakeAscii(szShapeFile),
                convertToFakeAscii(szAccess));
        }

        [DllImport("Shapelib.dll", CharSet = CharSet.Auto)]
        public static extern IntPtr getQuadTree(string fileName, string pszAccess);
        ...
        /// <summary>
        /// Unicode version of SHPCreate()
        /// </summary>
        /// <param name="szShapeFile"></param>
        /// <param name="shpType"></param>
        /// <returns></returns>
        [DllImport("Shapelib.dll", CharSet = CharSet.Auto)]
        public static extern IntPtr SHPCreateW(string szShapeFile, ShapeType shpType);
		...
		/// <returns>Entity number of the new shape</returns>
		[DllImport("Shapelib.dll", CharSet=CharSet.Auto)]
		...
		/// <summary>
		/// The DBFOpenW() function should be used to establish access to an existing xBase format table file. 
		...
		/// <returns>IntPtr</returns>
		[DllImport("Shapelib.dll", CharSet=CharSet.Auto)]
		public static extern IntPtr DBFOpenW (string szDBFFile, string szAccess);
		...
		/// <returns>IntPtr</returns>
		[DllImport("Shapelib.dll", CharSet=CharSet.Auto)]
		public static extern IntPtr DBFCreateW (string szDBFFile);
		...
		[DllImport("Shapelib.dll", CharSet=CharSet.Auto)]
		public static extern DBFFieldType DBFGetFieldInfoW (IntPtr hDBF, int iField, 
		...
		[DllImport("Shapelib.dll", CharSet=CharSet.Auto, EntryPoint="DBFReadDateAttribute")]
		...
		/// </remarks>
		[DllImport("Shapelib.dll", CharSet=CharSet.Auto)]
		public static extern string DBFReadStringAttributeW (IntPtr hDBF, int iShape, int iField);

		[DllImport("Shapelib.dll", CharSet=CharSet.Auto, EntryPoint="DBFReadLogicalAttribute")]
		...
		[DllImport("Shapelib.dll", CharSet=CharSet.Auto, EntryPoint="DBFWriteDateAttribute")]
		...
		/// <returns>int</returns>
		[DllImport("Shapelib.dll", CharSet=CharSet.Auto)]
		public static extern int DBFWriteStringAttributeW (IntPtr hDBF, int iShape, 
		...
		[DllImport("Shapelib.dll", CharSet=CharSet.Auto, EntryPoint="DBFWriteLogicalAttribute")]
		...
		/// <returns>IntPtr</returns>
		[DllImport("Shapelib.dll", CharSet=CharSet.Auto)]
		public static extern IntPtr DBFCloneEmptyW (IntPtr hDBF, string szFilename);
